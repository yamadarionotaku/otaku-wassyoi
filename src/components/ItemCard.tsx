...
import type { Availability, ItemWithCharacter } from "@/types";
...
export function ItemCard({ item }: ItemCardProps) {
  return (
    <Link
      href={`/items/${item.id}`}
      className="group block overflow-hidden rounded-[1.75rem] border border-zinc-200/80 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-amber-200 hover:shadow-md"
    >
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-2">
            ...
            <h2 className="line-clamp-2 text-lg font-bold leading-7 text-zinc-950">
              {item.title_ja}
            </h2>
          </div>
          <span
