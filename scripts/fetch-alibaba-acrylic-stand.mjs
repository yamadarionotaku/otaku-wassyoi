...
const QUERY = "anime acrylic stand";
const SEARCH_URL = "https://www.alibaba.com/trade/search?SearchText=anime+acrylic+stand";
const SHOWROOM_URL = "https://www.alibaba.com/showroom/anime-acrylic-stand.html";
const OUTPUT_PATH = "/home/yamadarion/projects/otaku-wassyoi/data/alibaba-acrylic-stand.json";
const USER_AGENT =
...
function looksBlocked(html) {
  return /captcha interception|unusual traffic|please slide to verify|punish-component/i.test(html);
}
...
async function resolveProducts() {
  ...
  const showroomHtml = runCurl(SHOWROOM_URL);
  attempts.push({
    method: "curl",
    url: SHOWROOM_URL,
  ...
  products = extractProducts(pageData);
  if (products.length > 0) {
    return { source: "showroom-curl", url: SHOWROOM_URL, attempts, products };
  }
